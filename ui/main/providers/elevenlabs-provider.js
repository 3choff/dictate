/**
 * ElevenLabs Provider
 * Streaming transcription using ElevenLabs Scribe v2 Realtime WebSocket API
 */
...
export class ElevenLabsProvider extends BaseProvider {
    ...
    async start(audioCaptureManager, visualizer) {
        ...
        // Start audio capture with ElevenLabs callback
    ...
    async stop() {
        ...
        if (this.sessionId) {
            try {
                ...
                console.error('[ElevenLabs] Failed to stop session:', error);
            }
            this.sessionId = null;
        }
